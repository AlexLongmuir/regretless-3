...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (!token) {
      console.log('❌ [ACCOUNT DELETE API] Unauthorized - no token');
    ...
    if (!user.data.user) {
      console.log('❌ [ACCOUNT DELETE API] Unauthorized - invalid token');
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }
    ...
    if (occurrencesError) {
      console.log('❌ [ACCOUNT DELETE API] Error deleting occurrences:', occurrencesError);
      // Continue anyway
    }
    ...
    if (actionsDeleteError) {
      console.log('❌ [ACCOUNT DELETE API] Error soft deleting actions:', actionsDeleteError);
    ...
    if (areasDeleteError) {
      console.log('❌ [ACCOUNT DELETE API] Error soft deleting areas:', areasDeleteError);
    ...
    if (dreamsDeleteError) {
      console.log('❌ [ACCOUNT DELETE API] Error soft deleting dreams:', dreamsDeleteError);
    ...
    if (notificationsError) {
      console.log('❌ [ACCOUNT DELETE API] Error deleting notification preferences:', notificationsError);
    ...
    if (aiEventsError) {
      console.log('❌ [ACCOUNT DELETE API] Error deleting AI events:', aiEventsError);
    ...
    if (profileError) {
      console.log('❌ [ACCOUNT DELETE API] Error deleting profile:', profileError);
    ...
    if (authError) {
      console.log('❌ [ACCOUNT DELETE API] Error deleting auth user:', authError);
