...
import { Modal, View, Text, StyleSheet, ScrollView, TouchableOpacity, Alert, FlatList, Animated, Dimensions } from 'react-native';
...
interface FigurineSelectorSheetProps {
  visible: boolean;
  onClose: () => void;
  onSelect?: (figurineUrl: string) => void;
}

export const FigurineSelectorSheet: React.FC<FigurineSelectorSheetProps> = ({ 
  visible, 
  onClose,
  onSelect 
}) => {
  ...
  // Load precreated figurines and current figurine
  useEffect(() => {
    if (!visible) return;
  ...
  const processImage = async (asset: ImagePicker.ImagePickerAsset) => {
    ...
    // Temporarily clear existing figurine to show uploaded image during generation
    const previousFigurine = userGeneratedFigurine;
    setUserGeneratedFigurine(null);
    try {
      const { data: { session } } = await supabaseClient.auth.getSession();
      if (session?.access_token) {
        ...
        if (uploadResponse.success && uploadResponse.data) {
          ...
          // Save to profile and notify
          await handleFigurineSelect(generatedFigurine.signed_url);
        } else {
          Alert.alert('Error', uploadResponse.message || 'Failed to generate figurine. Please try again.');
          // Restore previous figurine on error
          if (previousFigurine) {
            setUserGeneratedFigurine(previousFigurine);
          }
        }
      }
    } catch (error) {
    ...
    try {
      ...
      if (user) {
        ...
        if (error) {
          ...
          Alert.alert('Error', 'Failed to save figurine. Please try again.');
          return;
        }

        // Notify parent component
        if (onSelect) {
          onSelect(figurineUrl);
        }
      }
    } catch (error) {
      console.error('Error saving figurine to profile:', error);
  ...
  const handleDefaultFigurineSelect = (figurine: Figurine) => {
    setUserGeneratedFigurine(figurine);
    handleFigurineSelect(figurine.signed_url);
  };

  const renderDefaultFigurine = ({ item }: { item: Figurine }) => {
    return (
      <TouchableOpacity
        onPress={() => handleDefaultFigurineSelect(item)}
        ...
        <SafeAreaView style={styles.safeArea} edges={['top']}>
          ...
          >
            ...
            {isGenerating && uploadedSelfieUri ? (
              // Show uploaded image with generating overlay
